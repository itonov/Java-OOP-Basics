    ...
    public String startRace() {
        ...
        if (this.winners.size() >= 3) {
            this.winners = this.winners.stream()
                    .sorted(Comparator.comparingInt(Car::calculateSuspensionPerformance).reversed())
                    .limit(3)
            ...
            sb.append("1. ").append(firstCar.getBrand()).append(" ").append(firstCar.getModel()).append(" ")
                    .append(firstCar.calculateSuspensionPerformance()).append("PP - $").append(super.calculateFirstPrize())
                    .append(System.lineSeparator())
                    .append("2. ").append(secondCar.getBrand()).append(" ").append(secondCar.getModel()).append(" ")
                    .append(secondCar.calculateSuspensionPerformance()).append("PP - $").append(super.calculateSecondPrize())
                    .append(System.lineSeparator())
                    .append("3. ").append(thirdCar.getBrand()).append(" ").append(thirdCar.getModel()).append(" ")
                    .append(thirdCar.calculateSuspensionPerformance()).append("PP - $").append(super.calculateThirdPrize());
        } else {
            this.winners = this.winners.stream()
                    .sorted(Comparator.comparingInt(Car::calculateSuspensionPerformance).reversed())
                    .collect(Collectors.toList());
            if (this.winners.size() == 1) {
                ...
                sb.append("1. ").append(winner.getBrand()).append(" ").append(winner.getModel()).append(" ")
                        .append(winner.calculateSuspensionPerformance()).append("PP - $").append(super.calculateFirstPrize());
            } else {
                Car firstCar = this.winners.get(0);
                Car secondCar = this.winners.get(1);
                sb.append("1. ").append(firstCar.getBrand()).append(" ").append(firstCar.getModel()).append(" ")
